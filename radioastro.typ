...
== Radio Interferometer Measurement Equation, RIME @Smirnov_2011
=== Résumé 

Un point observé dans le ciel $e=mat(e_x ; e_y)$ subit des distorsions (optiques, électroniques...) supposées linéaires. On les représente par des matrices de Jones : $e'=J e$ qui se cumulent : $e'=J_n J_{n-1}...J_1 e = J e$ et se simplifient en une matrice de Jones totale. La mesure de ce signal par une antenne donne une tension complexe (ou vectorielle) $v=mat(v_a ; v_b)=J e$ 

=== Fonction du signal lumineux venu du ciel 

Si deux antennes p et q mesurant des vecteurs de tension $v_p, v_q$, on peut corréler leurs mesures pour former un interféromètre et obtenir une matrice de visibilités : $V_"pq" = 2 mat(<v_"pa" v*_"qa">,  <v_"pa" v*_"qb"> ; <v_"pb" v*_"qa", <v_"pb" v*_"qb">) =2<v_p v_q^H>$ où $<>$ dénote une moyenne du signal sur un temps (court, quelques secondes, 8 par exemple). 
...
On arrive à $V_"pq" = integral_{4\pi} J_p(sigma) B(sigma) J_q^H(sigma) d Omega $ intégrale sur une sphère unité. L'intégrale n'est pas pratique donc on fait une projection sinusoidale dans le plan (l,m) : $V_"pq" = integral_l integral_{m} J_p(l) B #strong("l") J_q^(H #strong("l")) (d l d m)"/" n $ 
...
Note que chaque baseline voit donc un ciel différent, à moins que tous les effets DD soient égaux : $E_(p(#strong("l"))=E(#strong("l"))$.
